...
const OUTCOME = ['Sam wins!', 'Draw!', 'Mat wins!'];
...
class App extends React.Component{
    ...
    async componentDidMount(){
        ...
        const balance = reach.formatCurrency(balanceAtomic, 3);
    ...
    async fundAccount(fundAmount){
        ...
        this.state({view: 'DeployerOrAttacher'})
    }
    async skipFundAccount() {this.state({view: 'DeployerOrAttacher'});}
...
class Player extends React.Component{
    ...
    seeCorrectAns(ans) {this.setState({view: 'Answer', answer: GUESS[ans]});}
    seeOutcome(a) {this.setState({view: 'Done', outcome: OUTCOME[a]});}
...
class Deployer extends Player{
    ...
    async deploy(){
        const ctc = this.props.account.contract(backend);
...
class Attacher extends Player{
    ...
    attach(ctcInfoStr){
        const ctc = this.props.account.contract(backend, JSON.parse(ctcInfoStr));
        this.setState({view: 'Attaching'});
        backend.Mat(ctc, this);
    }
    async acceptWager(wagerAtomic){
        const wager = reach.formatCurrency(wagerAtomic, 3);
